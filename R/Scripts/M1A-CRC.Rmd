...
```{r Prelims, include=FALSE, echo=FALSE, results='hide', message=FALSE} 
 ...
 figure_DIR <- file.path(WRKDIR, 'Scripts', 'figure/M1A/')
 suppressMessages(dir.create(figure_DIR, recursive=T))
 opts_chunk$set(fig.path=figure_DIR)

 #table_DIR <- file.path(WRKDIR, 'Scripts', 'table/M1A/')
 ...
 if(file.exists('/Users/francois/Dropbox/PredictiveModeling/CC/'))
 ROOT <- '/Users/francois/Dropbox/PredictiveModeling/CC/'  else
 ROOT <- '/mnt100/home/Dropbox/PredictiveModeling/CC/'
...
Approximately 15 % of colorectal carcinomas (CRC) display high level microsatellite instability (MSI-H) due to either a germline mutation in one of the genes responsible for DNA mismatch repair or somatic inactivation of the same pathway 
...
MSI status as part of clinical sample charactistics and use these data to illustrate some gene expression data analysis steps:
...
into a single process.  This entire report, including all data downloads and
processing can be reproduced by running a single R markdown script.  Some of the
data analysis steps may be time comsuming, but  `kntr` provides easy ways to
deal with these contingencies through neat caching capabilities.
...
In this analysis we will simply use quantile normalization to renormalize the
data after pooling across datasets and verify that egregious batch effects are not present.  We could subsequently repeat this analysis using a more sophisticated form of normalization and batch correction and assess the benefits that it brings to the analysis results.  Such an assessment is never trivial.
...
estimates making reducing the probe set data to gene level estiamtes
...
or clustering performance.  When it comes to the intrepretation of a
particular predictive model, we can bring in the gene annotation associated
with each probe set at this point without (much) loss of generality.
...
pre-filter genes based on this information.
...
a reliable set of data to assess the performance of the predcitive models.
...
biologocal clusters very challenging.  On the other hand, the batch effect correction method may
...
```{r getGEOHelp, echo=FALSE, cache=TRUE, cache.vars='GEOquery.getGEO.path'}
...
[getGEO](`r GEOquery.getGEO.path`) from package `GEOquery`.


```{r dnldGSE, eval=FALSE, cache=FALSE, cache.vars=''}
...
```{r  peekGSE, eval=TRUE, cache=TRUE, cache.vars='', fig.width=11, fig.height=6}
...
```{r cleanGSE, echo=T, cache=TRUE, cache.vars='',eval=TRUE, fig.width=8, fig.height=6}
 # CHANGE THIS LINE TO CLEAR CACHE ..
...
```{r poolGSE,cache=TRUE,cache.vars='',fig.width=12, fig.height=6}
...
```{r svaBatchAdjust, cache=TRUE, cache.vars='',fig.width=12, fig.height=6}
...
```{r getTrainTest, cache=TRUE,cache.vars=''}
 # CHANGE THIS LINE TO CLEAR CACHE
...
```{r saveGeneSets,cache=TRUE,cache.vars=''}
 # CHANGE THIS LINE TO CLEAR CACHE
...
-->
```{r stepLDAFit, cache=TRUE,cache.vars='', eval=FALSE, echo=FALSE}
 # CHANGE THIS LINE TO CLEAR CACHE
 ################################# 
 set.seed(12379)
...
```{r stepLDAFitPrint,echo=F,eval=FALSE}
...
-->
```{r stepQDAFit, cache=TRUE,cache.vars='', eval=FALSE,echo=FALSE}
 # CHANGE THIS LINE TO CLEAR CACHE
 ################################# 
 set.seed(12379)
...
``` 
<!-- SHOULD HAVE PRINTED IN THE SAME CHUNK! -->
```{r stepQDAFitPrint,echo=F,eval=FALSE}
...
## knn - k nearest neighbors
```{r knnFit, cache=TRUE,cache.vars='', eval=TRUE}
 # CHANGE THIS LINE TO CLEAR CACHE
 ################################# 
 set.seed(12379)
...
```
<!-- SHOULD HAVE PRINTED IN THE SAME CHUNK! -->
```{r knnFitPrint,echo=F,eval=TRUE}
...
```{r pamFit, cache=TRUE,cache.vars='',eval=TRUE}
 set.seed(12379)
...
```
<!-- SHOULD HAVE PRINTED IN THE SAME CHUNK! -->
```{r pamFitPrint,echo=F,eval=TRUE}
...
## rf - Random forests
```{r rfFit, cache=TRUE,cache.vars='',eval=TRUE}
 set.seed(12379)
...
## svmRadial - Support vector machines (RBF kernel)
```{r svmRadialFit, cache=TRUE,cache.vars='',eval=TRUE}
 set.seed(12379)
...
```
<!-- SHOULD HAVE PRINTED IN THE SAME CHUNK! -->
```{r svmRadialFitPrint,echo=F,eval=TRUE}
...
## gbm - Boosted trees
```{r gbmFit, cache=TRUE,cache.vars='',eval=TRUE}
 set.seed(12379)
...
```
<!-- SHOULD HAVE PRINTED IN THE SAME CHUNK! -->
```{r gbmFitPrint,echo=F,eval=TRUE}
...
## xgbLinear - eXtreme Gradient Boosting
```{r xgbLinearFit, cache=TRUE,cache.vars='',eval=TRUE}
 set.seed(12379)
...
```
<!-- SHOULD HAVE PRINTED IN THE SAME CHUNK! -->
```{r xgbLinearFitPrint,echo=F,eval=TRUE}
...
## xgbTree - eXtreme Gradient Boosting
-->
```{r xgbTreeFit, cache=TRUE,cache.vars='',echo=FALSE, eval=FALSE}
 set.seed(12379)
...
```
<!-- SHOULD HAVE PRINTED IN THE SAME CHUNK! -->
```{r xgbTreeFitPrint,echo=F,eval=FALSE}
...
## Compare Times
```{r compTimes, cache=TRUE, cache.vars=''}
...
### Train Data Accuracy
```{r trainAccuracy, cache=TRUE, cache.vars=''}
 suppressMessages(require(caret))
...
```{r OOSTrainAccuracy, cache=TRUE, cache.vars=''}
...
### Test Data Accuracy
```{r testAccuracy, cache=TRUE, cache.vars=''}
 suppressMessages(require(caret))
...
### Training Data ROC
```{r CompROCTrain, cache=TRUE, cache.vars='Set.Train.roc.mtx.lst', fig.height=6, fig.width=11}
 # CHANGE THIS LINE TO CLEAR CACHE
...
```{r CompROCOOSTrain, cache=FALSE, cache.vars='', fig.height=6, fig.width=11}
 suppressMessages(require(pROC))
 # Get predicted probabilities
...
```{r CompROCIndivOOSTrain, cache=FALSE, cache.vars='Set.Train.indiv_osProbClass1.mtx', fig.height=6, fig.width=11}
 suppressMessages(require(pROC))
...
```{r CompOOSSet.TrainProbClass1, cache=FALSE, cache.vars='', fig.height=12, fig.width=12}
...
```{r varImpHelp, echo=FALSE, cache=TRUE, cache.vars='caret.varImp.path'}
 # CHANGE TO CLEAR CACHE .
 caret.varImp.path <- file.path(help_DIR, 'caret.varImp.thml')
...
```{r ViewPamCluster, fig.width=12, fig.height=12}
...
```{r BagClust2, cache=T, cache.vars=''}
 suppressMessages(require(cluster))
 suppressMessages(require(stats))
...
```{r lkBagClust2, fig.height=12, fig.width=12}
 suppressMessages(require(cluster))
...
```{r ViewTsnePamCluster, fig.width=12, fig.height=12, results='hold'}
 suppressMessages(require(cluster))
 loadObj(paste0(SelGenes, '.tsne.lst'), 'SelGenes.tsne.lst')
...
```{r tsneBagClust2, cache=T, cache.vars=''}
 suppressMessages(require(cluster))
 suppressMessages(require(stats))
...
```{r lktsneBagClust2, fig.height=12, fig.width=12}
 suppressMessages(require(cluster))
...
<!-- To run
# sudo nohup Rscript -e "knitr::knit2html('M1A-CRC.Rmd')" > M1A-CRC.log  &

# Or
# sudo nohup Rscript -e "rmarkdown::render('M1A-CRC.Rmd')" > M1A-CRC.log  &
